...
class templateadmintools {
    ...
    public static function fetch_template_table() {
        ...
        $table->id = 'filter_videoeasy_template_list';
        $table->head = array(
            get_string('name'),
            get_string('version'),
        ...
        foreach ($template_details as $item) {
            ...
            if($updateversion) {
                $button =  new \single_button(
                    new \moodle_url($CFG->wwwroot . '/filter/videoeasy/videoeasytemplatesadmin.php',array('updatetemplate'=>$item->index)),
                    get_string('updatetoversion','filter_videoeasy',$updateversion));
        ...
        if($have_updates){
            $all_button =  new \single_button(
                new \moodle_url($CFG->wwwroot . '/filter/videoeasy/videoeasytemplatesadmin.php',array('updatetemplate'=>-1)),
                get_string('updateall','filter_videoeasy'));
    ...
    public static function fetch_template_details($conf){
        ...
        if($conf && property_exists($conf,'templatecount')){
            ...
            $templatecount = videoeasy_utils::FILTER_VIDEOEASY_TEMPLATE_COUNT;
        }
        for($tindex=1;$tindex<=$templatecount;$tindex++) {
            ...
            $template_details->url = new \moodle_url( '/admin/settings.php', array('section'=> 'filter_videoeasy_templatepage_' . $tindex));
